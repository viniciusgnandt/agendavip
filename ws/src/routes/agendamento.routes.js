...
router.post('/', async (req, res) => {
  ...
  // SIMULAR TRANSAÇÃO SEM O PAGARME
  try {
    // INICIANDO TRANSAÇÃO
    const { clienteId, salaoId, servicoId, colaboradorId } = req.body;
    
    // RECUPERANDO DADOS NECESSÁRIOS PARA PAGAMENTO
...
// OBTER DIAS DISPONÍVEIS PARA AGENDAMENTO
    ...
    for (let i = 0; i <= 365 && agenda.length <= 7; i++) {
      ...
      if (espacosValidos.length > 0) {
        ...
        for (let colaboradorKey of Object.keys(todosHorariosDia)) {
          ...
          //VERIFICANDO OS HORÁRIOS DENTRO DO SLOT QUE TENHAM A CONTINUIDADE NECESSÁRIA DO SERVIÇO
          ...
          // SEPARANDO 2 EM 2
          horariosLivres = horariosLivres.flat();
